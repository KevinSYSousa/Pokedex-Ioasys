...
import SharedButton from '../ShareButton/ShareButton';

const DetailsCard = ({action, pokemon, details}) => {
	...
	const getData = async () => {
		...
		if (!data) return;
		const found = data.find((item) => item.id === pokemon.id);
		if (found) {
			setLike(true);
		} else {
			setLike(false);
	...
	const handleLike = async () => {
		setWarn(true);
		const result = await saveFavorite('favorite', pokemon);
		switch (result) {
			case 'success':
				setLike(true);
		...
		<>
			<StyledContainer>
				...
				<StyledCardContainer nightMode={nightMode}>
					<StyledLikeContainer>
						<SharedButton nightMode={nightMode} onPress={action} />
						<StyledLikeButton
							onPress={() => {
